...
清理工具最大的隐私风险不是"删错"，是"为了判断要不要删，先把你的聊天记录、笔记、邮件全读一遍"。SDS扫地僧 的勘测路径是**只列目录名和文件名**，**绝不打开**聊天 DB、媒体文件、账号 key、配置文件等任何**用户产生的内容**。
...
→ "AI 误删"在 SDS扫地僧 不是被防住的，而是**actuation 路径上根本没有 AI 节点**。这是设计阶段的决定，不是事后加的栏杆。
