...
SDS扫地僧 干三件事：**画一张磁盘地图 → 让你拖文件夹问 AI → 给少数几个常见软件配齐安全清理脚本**。三件事都跑在你本机，文件内容**绝不**离开你的电脑。
...
SDS扫地僧 在 Windows 上换了个法子：**直接读图书馆的"目录卡片柜"**——这柜子叫 **NTFS Master File Table（主文件表，简称 MFT）**，里面早就记好了"哪本书放哪、多大、谁的"。一次性把整个柜子扫完就够了，不用一架架翻。整盘 C: 通常 **2–5 秒**。

> 没有 MFT 的系统（macOS / Linux / 移动盘 / 网盘）会自动 fallback 到"一架架翻"模式（基于一个叫 `jwalk` 的高性能并行库），比 MFT 慢但仍然比传统工具快。

扫完之后，SDS扫地僧 把每个文件夹的大小算出来，画成两张图：
...
SDS扫地僧 的中间是一个聊天框。**把不认识的文件夹拖进去**，AI 用人话告诉你：
...
2. **每次操作记到 `~/.saodiseng/undo.jsonl`**——一个文本日志（路径在你电脑用户目录下），能查到"我啥时候删了什么"
3. **可选 7 天隔离区**：把删的东西放到 `~/.saodiseng/quarantine/` 暂存 7 天再清。给特别敏感的删除一道额外保险
...
这些是 SDS扫地僧 **架构上保证的**，不是写在隐私政策里的"我们尽力"：
