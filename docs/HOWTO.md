...
```bash
cargo run -p saodiseng-scaffold-lint -- scaffolds/<id>.toml   # lint 单个
cargo test -p saodiseng-scaffold                               # 跑全部 safety test
cargo test -p saodiseng-scaffold --test <id>_safety            # 单个 safety test
...
- 推之前跑一遍 `cargo test -p saodiseng-scaffold` 兜底
