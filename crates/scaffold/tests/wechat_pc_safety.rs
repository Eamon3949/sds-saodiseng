...
fn load_wechat_pc() -> saodiseng_scaffold::Scaffold {
...
fn wechat_pc_matcher_distinguishes_data_from_installs() {
    ...
    let assert_match = |path: &Path, expected: Option<&str>, label: &str| {
        let got = saodiseng_scaffold::detect_for(&scaffolds, path);
        assert_eq!(
            got.as_deref(),
            expected,
    ...
    let compiled = saodiseng_scaffold::compile_all(&scaffolds);
    let assert_compiled = |path: &Path, expected: Option<&str>, label: &str| {
        let got = saodiseng_scaffold::detect_compiled(&compiled, path);
        assert_eq!(
            got.as_deref(),
            expected,
