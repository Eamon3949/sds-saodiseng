...
- [ ] **Phase 9**: `cargo run -p saodiseng-scaffold-lint -- scaffolds/<id>.toml` 通过
- [ ] **Phase 10**: `crates/scaffold/tests/<id>_safety.rs` 存在；`cargo test -p saodiseng-scaffold` 通过；含**红线断言**
- [ ] **Phase 11**: 若改了 `Scaffold`/`Scope` 等结构，`cargo check -p saodiseng-desktop` + `pnpm -C apps/desktop exec tsc --noEmit` 都干净
