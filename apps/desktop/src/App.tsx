...
export default function App() {
  ...
  const [leftWidth, setLeftWidth] = useState<number>(() => {
    const v = Number(localStorage.getItem('saodiseng.leftWidth'));
    return Number.isFinite(v) && v > MIN_LEFT ? v : DEFAULT_LEFT;
  });
  const [rightWidth, setRightWidth] = useState<number>(() => {
    const v = Number(localStorage.getItem('saodiseng.rightWidth'));
    return Number.isFinite(v) && v > MIN_RIGHT ? v : DEFAULT_RIGHT;
  });

  useEffect(() => { localStorage.setItem('saodiseng.leftWidth', String(leftWidth)); }, [leftWidth]);
  useEffect(() => { localStorage.setItem('saodiseng.rightWidth', String(rightWidth)); }, [rightWidth]);
  ...
  return (
    <div className="app">
      <header>
        <span className="brand"><Logo size={22} /> SDS扫地僧</span>
      ...
      <footer>
        <span>SDS扫地僧 v0.1.1 · {scaffolds.length} 个脚本</span>
...
function DiagnosticsBar({ diag }: { diag: ScanDiag }) {
  ...
  return (
    <div className="diag-bar" title="扫描各阶段耗时 — localStorage 开关：saodiseng.hideStudio">
